...
class ProductImageSql implements ProductImageSqlInterface
{
 ...
 ";
    ...
    FROM (
      ...
      spy_product_image_set.name = input.name AND
      fk_locale = fkLocale AND
      (fk_product = fkProduct OR fk_product_abstract = fkProductAbstract)
      INNER JOIN spy_product_image ON
      product_image_key = productImageKey
      LEFT JOIN spy_product_image_set_to_product_image ON 
      fk_product_image = id_product_image AND
      fk_product_image_set = id_product_image_set
),
    updated AS (
    UPDATE spy_product_image_set_to_product_image
    SET sort_order = records.sort_order
    FROM records
    WHERE id_product_image = fk_product_image AND id_product_image_set = fk_product_image_set
  ),
    inserted AS(
