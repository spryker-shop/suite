...
function doesDatabaseHaveTables()
{
    tableCount=$(execSprykerMultiString <<'EOF'
        ...
        psql -lqt -h ${SPRYKER_DB_HOST} -U ${SPRYKER_DB_USERNAME} | grep ${SPRYKER_DB_DATABASE} > /dev/null 2>&1 \
            && PGPASSWORD="${SPRYKER_DB_PASSWORD}" psql -h ${SPRYKER_DB_HOST} -U ${SPRYKER_DB_USERNAME} ${SPRYKER_DB_DATABASE} -c "SELECT count(*) FROM information_schema.tables WHERE table_catalog = '${SPRYKER_DB_DATABASE}';" -t 2>&1 | tr -d ' \n\r' \
            || echo 0
EOF
)
